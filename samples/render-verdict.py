...
This is a standalone public formatter, not a signature verifier or the
superseded `provenex-ingest` binary's report path.
...
def render(source_label: str, verdict: dict, version: str = "public-renderer-v1") -> str:
    ...
    badge = "RED" if red > 0 else "NO RED"
    ...
    if use_narrative:
        ...
        n = len(findings)
        if red > 0:
            headline = f"We found {n} Red chain finding{'s' if n != 1 else ''}"
        else:
            headline = "No Red chain findings in this trace"
        ...
        findings_section = (
            ...
            if cards
            else '<h2>Findings (0)</h2><p style="color: #555; font-style: italic;">No Red chain findings under the returned evidence and policy.</p>'
        )
    else:
        # Legacy fallback — preserves the previous binding-reason-headline layout.
        headline = (
            (verdicts[0].get("binding_reason") if verdicts else None)
            or ("No Red chain findings in the returned result." if red == 0 else "Cross-zone composition detected.")
        )
        outcome = verdict.get("ingest_outcome") or {}
        receipts_ingested = verdict.get("receipts_ingested") or 0
        egress_points_seen = outcome.get("egress_points_seen") or 0

        legacy_cards = [render_legacy_card(vd, i) for i, vd in enumerate(verdicts)]
        body_cards = "".join(legacy_cards)
        stats_html = (
            f'<div class="stats">'
            f'<div class="stat"><div class="stat-label">Lineage receipts ingested</div>'
            f'<div class="stat-value">{receipts_ingested}</div></div>'
            f'<div class="stat"><div class="stat-label">Egress points evaluated</div>'
            f'<div class="stat-value">{egress_points_seen}</div></div>'
...
<footer>
  ...
  <p style="margin-top: 1rem; font-size: 0.85rem; color: #888;">
    The Engine signs stored verdict artifacts, but this POST summary omits the
    signature envelope and this local report does not verify it. Use the
    authenticated <code>/v1/health/key</code> response only to inspect the tenant's
    advertised key identity. A Red finding is a detection result, not proof that an
    action was blocked; no Red is not proof of complete telemetry coverage.
  </p>
</footer>
</body>
</html>
